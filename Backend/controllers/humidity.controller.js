...
// // Hàm lấy dữ liệu mới nhất từ MQTT
// const fetchLatestSensorData = (feed) => {
//   return new Promise((resolve) => {
//     const topic = `${process.env.ADAFRUIT_USERNAME}/feeds/${feed}`;
//     adafruitService.client.subscribe(topic, (err) => {
//       if (err) {
//         console.error(`Lỗi khi subscribe tới ${topic}:`, err);
//         resolve(null);
//         return;
//       }
//       adafruitService.client.on("message", (receivedTopic, message) => {
//         if (receivedTopic === topic) {
//           const value = parseFloat(message.toString());
//           if (!isNaN(value)) {
//             console.log(`Nhận giá trị từ ${topic}: ${value}`);
//             resolve(value);
//           } else {
//             console.error(`Giá trị từ ${topic} không phải số: ${message.toString()}`);
//             resolve(null);
//           }
//           adafruitService.client.unsubscribe(topic, (err) => {
//             if (err) console.error(`Lỗi khi unsubscribe từ ${topic}:`, err);
//           });
//         }
//       });
//     });
